...
export const CreateDealModal: React.FC<{ leads: Lead[], onClose: () => void, onSave: (deal: Deal) => void }> = ({ leads, onClose, onSave }) => {
    ...
    const createDealMutation = trpc.crm.deals.create.useMutation({
        onSuccess: (deal) => {
            // Transform to local Deal type for onSave callback
            const selectedLead = leads.find(l => l.id === form.leadId);
            const newDeal: Deal = {
                id: deal.id,
                title: deal.title || form.title,
                value: deal.value ? String(deal.value) : form.value,
                stage: (deal.stage as Deal['stage']) || 'Prospect',
                company: selectedLead?.company || 'Unknown',
                leadId: deal.leadId || form.leadId,
                expectedClose: deal.expectedCloseDate?.toISOString().split('T')[0] || form.expectedClose,
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await createDealMutation.mutateAsync({
                ...
                expectedCloseDate: form.expectedClose ? new Date(form.expectedClose) : undefined,
                stage: form.stage,
            });
        } catch (err) {
    ...
    return (
        <div className="fixed inset-0 bg-charcoal/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in">
            <div className="bg-white w-full max-w-lg rounded-[2rem] p-8 shadow-2xl relative">
                <button onClick={onClose} disabled={isSubmitting} className="absolute top-6 right-6 text-stone-400 hover:text-charcoal transition-colors disabled:opacity-50">
                    <X size={24} />
                </button>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        ...
                        <input required disabled={isSubmitting} className="w-full p-3 bg-stone-50 border border-stone-200 rounded-xl focus:outline-none focus:border-rust disabled:opacity-50" placeholder="e.g. Q4 Staffing Contract" value={form.title} onChange={e => setForm({...form, title: e.target.value})} />
                    </div>

                    <div>
                        <label className="block text-xs font-bold text-stone-400 uppercase tracking-widest mb-2">Related Lead</label>
                        <select disabled={isSubmitting} className="w-full p-3 bg-stone-50 border border-stone-200 rounded-xl focus:outline-none focus:border-rust disabled:opacity-50" value={form.leadId} onChange={e => setForm({...form, leadId: e.target.value})}>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input disabled={isSubmitting} className="w-full p-3 bg-stone-50 border border-stone-200 rounded-xl focus:outline-none focus:border-rust disabled:opacity-50" placeholder="$0" value={form.value} onChange={e => setForm({...form, value: e.target.value})} />
                        </div>
                        <div>
                            <label className="block text-xs font-bold text-stone-400 uppercase tracking-widest mb-2">Close Date</label>
                            <input disabled={isSubmitting} type="date" className="w-full p-3 bg-stone-50 border border-stone-200 rounded-xl focus:outline-none focus:border-rust disabled:opacity-50" value={form.expectedClose} onChange={e => setForm({...form, expectedClose: e.target.value})} />
                        </div>
                    </div>

                    <div>
                        <label className="block text-xs font-bold text-stone-400 uppercase tracking-widest mb-2">Stage</label>
                        <select disabled={isSubmitting} className="w-full p-3 bg-stone-50 border border-stone-200 rounded-xl focus:outline-none focus:border-rust disabled:opacity-50" value={form.stage} onChange={e => setForm({...form, stage: e.target.value as typeof form.stage})}>
                    ...
                    <div className="flex gap-3 mt-6">
                        <button type="button" onClick={onClose} disabled={isSubmitting} className="flex-1 py-3 text-stone-500 font-bold text-xs uppercase tracking-widest hover:bg-stone-50 rounded-xl disabled:opacity-50">Cancel</button>
                        <button type="submit" disabled={isSubmitting} className="flex-1 py-3 bg-charcoal text-white font-bold text-xs uppercase tracking-widest rounded-xl hover:bg-rust transition-colors shadow-lg disabled:opacity-50 flex items-center justify-center gap-2">
                            {isSubmitting && <Loader2 className="w-4 h-4 animate-spin" />}
                            {isSubmitting ? 'Creating...' : 'Create Pipeline Deal'}
                        </button>
                    </div>
                </form>
            </div>
