...
export async function sendGraduationEmail(event: Event<CourseGraduatedPayload>) {
  const { studentId, courseName, grade, enrollmentId } = event.payload;
    ...
    await supabase.from('background_jobs').insert({
      ...
      payload: {
        ...
        data: {
          ...
          grade,
          certificate_url: `/students/certificates/${enrollmentId}`, // Will be generated by certificate handler
        },
      },
      priority: 'high',
      max_attempts: 3,
...
export async function generateCertificate(event: Event<CourseGraduatedPayload>) {
  const { studentId, courseName, grade, enrollmentId, courseId } = event.payload;
  ...
  try {
    ...
    const checklistItems = [
      ...
      },
      {
        ...
        description: 'Plan when you'll dedicate time to learning',
        sort_order: 5,
        is_completed: false,
      },
    ];
