# Events

⚙️ **Tier:** Operational  
**Last Updated:** 2025-11-21  
**Auto-Generated:** Yes (manual sections preserved)

---

## 📁 Purpose

Application events module.

---

## 📋 Contents

### Files in This Directory

- **CLAUDE.md** - Documentation

- **decorators.ts** (63 lines)
  Event Handler Decorators

- **EventBus.ts** (278 lines)
  Event Bus

- **HandlerRegistry.ts** (144 lines)
  Handler Registry

- **init.ts** (85 lines)
  Event Bus Initialization

- **types.ts** (106 lines)
  Event Bus Type Definitions


**Subdirectories:**
- `__tests__/` - See [__tests__/CLAUDE.md](__tests__/CLAUDE.md)
- `handlers/` - See [handlers/CLAUDE.md](handlers/CLAUDE.md)

---

## 🎯 Key Concepts

- Auto-discovered folder - add specific details in manual edit section

---

## 🔗 Dependencies

### This Folder Depends On:

- Parent: src/lib/

### Other Folders That Depend On This:

- To be documented

---

## 🚀 Quick Start

### For Developers

This folder contains events related files. Check CLAUDE.md for detailed documentation.

### For AI Agents

Auto-generated folder context. Review files in this directory for specific functionality.

---

## ⚠️ Important Notes

- ⚠️ This is an auto-generated CLAUDE.md - enhance in manual sections as needed

---

## 📝 Change Log

<!-- MANUAL EDIT SECTION - Add your changes below -->

### Recent Changes

- 2025-11-21: Auto-generated CLAUDE.md created

<!-- END MANUAL EDIT SECTION -->

---

## 📚 Related Documentation

- [Project Root CLAUDE.md](/CLAUDE.md)
- [Project Structure](/PROJECT-STRUCTURE.md)
- [Documentation Index](/.claude/DOCUMENTATION-INDEX.md)

- [Parent Folder](../lib/CLAUDE.md)

---

*Auto-generated on 2025-11-21 - Tier operational documentation*
