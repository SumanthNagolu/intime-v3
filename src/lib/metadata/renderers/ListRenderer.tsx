...
export function ListRenderer({ definition, className }: ListRendererProps) {
  ...
  // Extract data based on response shape (hotlists and job_orders return { items, total }, others return array)
  const rawData = query.data;
  const data = entityType === 'hotlist' || entityType === 'job_order'
    ? ((rawData as { items?: unknown[] } | undefined)?.items || [])
    : (rawData || []);
