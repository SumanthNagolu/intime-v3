  ...
  return (
    <div className={`flex ${compact ? 'h-full' : 'h-[calc(100vh-220px)]'} rounded-xl border border-charcoal-200/60 bg-white shadow-elevation-sm overflow-hidden`}>
      ...
      {!compact && <div className="w-64 border-r border-charcoal-100 flex flex-col bg-charcoal-50/50">
        <div className="p-4 border-b border-charcoal-100">
          <button
            ...
            className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg bg-charcoal-900 text-white text-sm font-medium transition-all hover:-translate-y-0.5 hover:shadow-lg"
          >
        ...
        <div className="flex-1 overflow-y-auto py-2">
          {sessions.length === 0 && (
            <div className="px-4 py-8 text-center">
              <Bot className="w-8 h-8 text-charcoal-300 mx-auto mb-2" />
          ...
          {sessions.map((session) => (
            ...
            >
              <div className="flex-1 min-w-0">
                <p className="text-sm text-charcoal-800 truncate font-medium">
                  {session.title}
                </p>
      ...
      <div className="flex-1 flex flex-col">
        {/* Header */}
        <div className="flex items-center gap-3 px-6 py-4 border-b border-charcoal-100">
          <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-gold-400 to-gold-600 flex items-center justify-center">
            <Sparkles className="w-5 h-5 text-white" />
          </div>
          <div>
            <h2 className="font-heading font-semibold text-charcoal-900 text-sm">
              Guidewire Guru
            </h2>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
          {!activeSession || activeSession.messages.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full space-y-6">
              <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-gold-100 to-gold-200 flex items-center justify-center">
                <Sparkles className="w-8 h-8 text-gold-600" />
              </div>
              <div className="text-center space-y-2">
                <h3 className="font-heading font-semibold text-charcoal-900 text-lg">
                  Guidewire Guru
                </h3>
              ...
              <div className="flex flex-wrap justify-center gap-2 max-w-lg">
                {suggestions.slice(0, 4).map((suggestion) => (
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-2 rounded-lg border border-charcoal-200 text-xs text-charcoal-600 hover:bg-charcoal-50 hover:border-charcoal-300 transition-all"
                  >
            ...
            <>
              {activeSession.messages.map((msg) => (
                ...
                >
                  ...
                  {msg.role === 'user' && (
                    <div className="w-8 h-8 rounded-lg bg-charcoal-200 flex items-center justify-center flex-shrink-0">
              ...
              {isThinking && (
                <div className="flex gap-3">
                  ...
                  <div className="bg-charcoal-50 rounded-xl px-4 py-3 border border-charcoal-100">
          ...
          <div className="flex items-end gap-3">
            <div className="flex-1 relative">
              <textarea
                ...
                rows={1}
                className="w-full rounded-lg border border-charcoal-200 bg-white px-4 py-3 pr-12 text-sm text-charcoal-800 placeholder:text-charcoal-400 focus:ring-2 focus:ring-gold-500/20 focus:border-gold-500 transition-all resize-none"
            ...
            <button
              ...
              className="flex items-center justify-center w-11 h-11 rounded-lg bg-charcoal-900 text-white transition-all hover:-translate-y-0.5 hover:shadow-lg disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:translate-y-0"
            >
