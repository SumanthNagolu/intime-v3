...
export function PodFormClient({ mode, podId, initialData }: PodFormClientProps) {
  ...
  // Load existing pod data for edit mode from server data
  useEffect(() => {
    if (mode === 'edit' && initialData.pod) {
      const pod = initialData.pod
      setName(pod.name)
  ...
  const filteredUsers = initialData.users.filter(
    (u) => {
      const matchesSearch = !userSearch ||
        u.full_name.toLowerCase().includes(userSearch.toLowerCase()) ||
        u.email.toLowerCase().includes(userSearch.toLowerCase())
      const notSelected = !selectedMembers.find(m => m.id === u.id)
      const notManager = u.id !== managerId
      return matchesSearch && notSelected && notManager
    }
  )
