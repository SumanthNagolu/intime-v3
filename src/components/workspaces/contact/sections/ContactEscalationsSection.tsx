...
import {
  ...
  X, ChevronLeft, ChevronRight, AlertCircle,
  Target, Zap, MessageSquare, Lightbulb, Timer,
  ExternalLink, BarChart3, TrendingUp, CalendarClock
} from 'lucide-react'
import {
  DropdownMenu,
  DropdownMenuContent,
...
import type { ContactEscalation, EscalationSeverity, EscalationStatus, EscalationType, ClientSatisfaction } from '@/types/workspace'
...
interface ContactEscalationsSectionProps {
...
/**
 * ContactEscalationsSection - Premium SaaS-level escalation management for contacts
 * Features: SLA tracking, severity indicators, urgency banner, detail panel
 */
export function ContactEscalationsSection({ escalations, contactId }: ContactEscalationsSectionProps) {
  const [selectedEscalation, setSelectedEscalation] = React.useState<ContactEscalation | null>(null)
  const [searchQuery, setSearchQuery] = React.useState('')
  const [statusFilter, setStatusFilter] = React.useState<StatusFilter>('all')
  const [currentPage, setCurrentPage] = React.useState(1)

  // Sync selectedEscalation with updated data when escalations prop changes
  const selectedEscalationId = selectedEscalation?.id
  React.useEffect(() => {
    if (selectedEscalationId) {
      const updated = escalations.find(e => e.id === selectedEscalationId)
      if (updated) {
  ...
  const handleRowClick = (escalation: ContactEscalation) => {
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-charcoal-500">
                  {filteredEscalations.length} escalation{filteredEscalations.length !== 1 ? 's' : ''} for this contact
                  {statusCounts.open > 0 && (
              ...
              <Button
                ...
                onClick={() => {
                  window.dispatchEvent(new CustomEvent('openContactDialog', {
                    detail: { dialogId: 'createEscalation', contactId }
                  }))
                }}
              >
...
function EscalationDetailPanel({ escalation, onClose }: { escalation: ContactEscalation; onClose: () => void }) {
  const severityConfig = SEVERITY_CONFIG[escalation.severity] || SEVERITY_CONFIG.medium
  const statusConfig = STATUS_CONFIG[escalation.status] || STATUS_CONFIG.open
  const typeConfig = TYPE_CONFIG[escalation.escalationType] || TYPE_CONFIG.other
  const SeverityIcon = severityConfig.icon
        ...
        <div className="mt-8 space-y-5">
          ...
          {!escalation.detailedDescription && !escalation.rootCause && !escalation.immediateActions && !escalation.resolutionPlan && (
            <div className="rounded-2xl border border-charcoal-100/50 p-8 bg-charcoal-50/50 flex flex-col items-center justify-center">
              ...
              <p className="text-xs text-charcoal-400 mt-1">Details will appear here once added</p>
            </div>
          )}
        </div>
...
export default ContactEscalationsSection
