...
export const AIMentor: React.FC = () => {
  const { isMentorOpen, setMentorOpen, mentorContext, setMentorContext } = useAppStore();
  const [isMinimized, setIsMinimized] = useState(false);
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      id: 'init',
      role: 'model',
      content: "Hi Priya! I'm your AI mentor. I'm here to guide you through your learning journey using the Socratic method. Instead of giving you direct answers, I'll ask questions to help you discover solutions yourself. What can I help you with today?",
      timestamp: new Date()
    }
  ]);
  ...
  useEffect(() => {
    if (isMentorOpen && !isMinimized) {
      scrollToBottom();
    }
  }, [messages, isMentorOpen, isMinimized]);

  // Handle Context Injection from Dashboard
  useEffect(() => {
    if (isMentorOpen && mentorContext) {
        const sendContext = async () => {
             ...
             setMentorContext(null); // Clear context after consuming
             
             setIsThinking(true);
             try {
                const history = messages.map(m => ({ role: m.role, content: m.content }));
                const responseText = await generateMentorResponse([...history, { role: 'user', content: userMsg.content }], userMsg.content);
  ...
  }, [isMentorOpen, mentorContext, messages, setMentorContext]);
    ...
    <div className={`fixed bottom-6 right-6 bg-white rounded-xl shadow-2xl border border-gray-200 z-50 flex flex-col transition-all duration-300 ${isMinimized ? 'w-72 h-14' : 'w-96 h-[600px]'}`}>
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 bg-white rounded-t-xl">
        ...
        <div className="flex items-center gap-2 text-gray-400">
          ...
          <button onClick={() => setMentorOpen(false)} className="hover:text-charcoal p-1">
            <X size={18} />
          </button>
        </div>
      </div>
