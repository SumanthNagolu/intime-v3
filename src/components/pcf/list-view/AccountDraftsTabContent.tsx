...
interface AccountDraftsTabContentProps {
  items: DraftItem[]
  isLoading: boolean
...
/**
 * Custom tab content for account drafts that:
 ...
 */
export function AccountDraftsTabContent({
  items,
  isLoading,
}: AccountDraftsTabContentProps) {
  const router = useRouter()
  const utils = trpc.useUtils()
  const deleteMutation = trpc.crm.accounts.deleteDraft.useMutation({
    onSuccess: () => {
      utils.crm.accounts.listMyDrafts.invalidate()
    },
  })

  const wizardRoute = '/employee/recruiting/accounts/new'
  ...
  return (
    <div className="space-y-3">
      {items.map((draft) => {
        ...
        const displayName = String(draft.name || 'Untitled')
