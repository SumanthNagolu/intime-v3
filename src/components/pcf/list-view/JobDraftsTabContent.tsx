...
import { FileText, Trash2, Play, Clock, Plus, Briefcase } from 'lucide-react'
...
interface JobDraftsTabContentProps {
  items: DraftItem[]
  isLoading: boolean
...
/**
 * Custom tab content for job drafts that:
 ...
 */
export function JobDraftsTabContent({
  items,
  isLoading,
}: JobDraftsTabContentProps) {
  const router = useRouter()
  const utils = trpc.useUtils()
  const deleteMutation = trpc.ats.jobs.deleteDraft.useMutation({
    onSuccess: () => {
      utils.ats.jobs.listMyDrafts.invalidate()
    },
  })

  const wizardRoute = '/employee/recruiting/jobs/new'
    ...
    return (
      <div className="text-center py-12 bg-charcoal-50 rounded-lg border border-dashed border-charcoal-200">
        ...
        <p className="text-sm text-charcoal-500 mb-4">
          You don&apos;t have any jobs in progress. Start creating a new one!
        </p>
  ...
  return (
    <div className="space-y-3">
      {items.map((draft) => {
        ...
        const displayName = String(draft.title || 'Untitled Job')
