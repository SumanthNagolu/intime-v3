...
export default function OnboardingChecklistPage() {
  ...
  const handleComplete = () => {
    if (selectedTask) {
      completeMutation.mutate({ taskId: selectedTask, notes: notes || undefined })
    }
  }

  const handleSkip = () => {
    if (selectedTask && skipReason) {
      skipMutation.mutate({ taskId: selectedTask, reason: skipReason })
    }
  }

  const handleAddNote = () => {
    if (selectedTask && notes) {
      addNoteMutation.mutate({ taskId: selectedTask, note: notes })
    }
  }
  ...
  const openSkipDialog = (taskId: string) => {
    setSelectedTask(taskId)
      ...
      <div className="min-h-screen bg-cream p-6">
        <Card>
          <CardContent className="py-12 text-center">
            ...
            <Button onClick={() => router.push('/employee/hr/onboarding')}>
              Back to Onboarding
            </Button>
          </CardContent>
        </Card>
    ...
    <div className="min-h-screen bg-cream p-6 space-y-6">
      ...
      <div className="flex items-center gap-2 text-body-sm text-charcoal-500">
        <span
          ...
          onClick={() => router.push('/employee/hr/onboarding')}
        >
          Onboarding
        </span>
          ...
          <div className="flex items-start justify-between">
            <div className="flex items-start gap-4">
              <Avatar className="h-16 w-16">
                <AvatarFallback className="bg-charcoal-200 text-charcoal-700 text-lg">
                  ?
                </AvatarFallback>
              </Avatar>
              <div>
                    ...
                    >
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          ...
                          {task.description && (
                            <p className="text-body-sm text-charcoal-500 mt-1">{task.description}</p>
                          )}
                          {task.skipReason && (
                            <p className="text-body-sm text-charcoal-500 mt-1 italic">
                          ...
                          {task.completedAt && (
                            <p className="text-caption text-charcoal-400 mt-1">
                              ...
                              {new Date(task.completedAt as string).toLocaleDateString()} by{' '}
                              {task.completedBy?.full_name ?? 'Unknown'}
                            </p>
                          )}
                        </div>
                      </div>
                        ...
                        {task.status === 'pending' && (
                          <div className="flex items-center gap-1">
                            <Button
                              variant="ghost"
                              size="sm"
                              onClick={() => openNoteDialog(task.id as string)}
                            >
                            ...
                            <Button
                              variant="outline"
                              size="sm"
                              onClick={() => openSkipDialog(task.id as string)}
                            >
                              <SkipForward className="h-4 w-4 mr-1" />
                              Skip
                            </Button>
                            <Button size="sm" onClick={() => openCompleteDialog(task.id as string)}>
      ...
      <Dialog open={showNoteDialog} onOpenChange={setShowNoteDialog}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleAddNote} disabled={!notes || addNoteMutation.isPending}>
