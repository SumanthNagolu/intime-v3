...
import { CreateDealDialog, CloseWonDialog, CloseLostDialog } from '@/components/crm/deals'
...
export default function DealsPage() {
  ...
  // Fetch pipeline data
  const { data: pipelineData, isLoading, refetch } = trpc.crm.deals.pipeline.useQuery({
          ...
          <div className="flex items-start justify-between gap-2">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                <DropdownMenuItem onClick={() => router.push(`/employee/crm/deals/${deal.id}`)}>
                  <Eye className="h-4 w-4 mr-2" />
                  View Details
                </DropdownMenuItem>
                <DropdownMenuSeparator />
                <DropdownMenuItem onClick={() => setCloseWonDeal(deal)}>
                  <Trophy className="h-4 w-4 mr-2 text-green-600" />
                  Close Won
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => setCloseLostDeal(deal)}>
