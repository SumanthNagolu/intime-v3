-- Migration 008: Complete - Add all missing columns and functions

-- 1. Add all missing columns to event_subscriptions
...
-- 3. Make org_id NOT NULL and add constraints
ALTER TABLE event_subscriptions
ALTER COLUMN org_id SET NOT NULL,
ADD CONSTRAINT IF NOT EXISTS event_subscriptions_org_id_fkey
...
COMMENT ON COLUMN event_subscriptions.last_failure_at IS 'Timestamp of last failure';
COMMENT ON COLUMN event_subscriptions.last_failure_message IS 'Error message from last failure';
COMMENT ON COLUMN event_subscriptions.auto_disabled_at IS 'Timestamp when handler was auto-disabled';
