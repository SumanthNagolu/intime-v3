#!/usr/bin/env python3
"""
Extract slide images for the missing chapters (ch09, ch11, ch13, ch14).
Uses the same extraction logic as extract-slide-images.py but points to
/Documents/Material and only processes the missing lessons.
"""
...
def extract_slide_images(pptx_path, output_dir):
    ...
    for idx, slide in enumerate(prs.slides):
        ...
        try:
            canvas = Image.new('RGB', (img_width, img_height), (250, 248, 244))
...
def main():
    print("=" * 60)
    print("MISSING SLIDE EXTRACTION")
    print(f"Source: {MATERIAL_DIR}")
    print("=" * 60)
    ...
    for chapter_name, info in CHAPTER_MAP.items():
        ...
        if not os.path.isdir(chapter_dir):
            print(f"\n  WARNING: Chapter dir not found: {chapter_dir}")
            continue

        index_path = os.path.join(CONTENT_DIR, slug, "index.json")
        if not os.path.exists(index_path):
            print(f"\n  WARNING: No index.json for {slug}")
            continue
        ...
        for lesson in index_data.get("lessons", []):
            ...
            if os.path.isdir(out_dir) and len([f for f in os.listdir(out_dir) if f.endswith('.png')]) > 0:
                existing = len([f for f in os.listdir(out_dir) if f.endswith('.png')])
                print(f"  Lesson {lesson_num}: SKIP (already has {existing} slides)")
                skipped_lessons += 1
                continue

            lesson_json = os.path.join(CONTENT_DIR, slug, f"lesson-{lesson_num}.json")
            if not os.path.exists(lesson_json):
                print(f"  Lesson {lesson_num}: SKIP (no lesson JSON)")
                continue
            ...
            if not source_file:
                print(f"  Lesson {lesson_num}: SKIP (no sourceFile)")
                continue

            # Find the PPTX file - try multiple strategies
            pptx_path = None

            # Strategy 1: Direct path with source_folder
            if source_folder:
            ...
            if not pptx_path or not os.path.exists(pptx_path):
                print(f"  Lesson {lesson_num}: SKIP (PPT not found: {source_file})")
                continue

            print(f"  Lesson {lesson_num}: {source_file}")
            count = extract_slide_images(pptx_path, out_dir)
            total += count
            extracted_lessons += 1
            print(f"    -> {count} slides extracted to {os.path.relpath(out_dir, SLIDES_DIR)}")

    print(f"\n{'=' * 60}")
    print(f"Extracted: {extracted_lessons} lessons, {total} total slides")
    print(f"Skipped: {skipped_lessons} lessons (already had slides)")
