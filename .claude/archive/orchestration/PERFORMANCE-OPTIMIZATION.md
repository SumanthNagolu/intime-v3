...
### Current Setting
- **Token Limit:** 200,000 (Claude API maximum)
...
**Recommended:** Keep at 200k (Claude API max)

**Why:** 200k is already the maximum Claude API allows. Can't go higher.
...
Read src/lib/db/schema/CLAUDE.md first for conventions."
...
1. **Agent reads directory_tree on large project**
   - **Solution:** Use folder CLAUDE.md instead
   - **Performance:** 10x faster

2. **Agent reads too many files at once**
   - **Solution:** Read folder CLAUDE.md, then only necessary files
...
```typescript
// .claude/orchestration/core/agent-runner.ts
...
**How:** Agents read folder CLAUDE.md first
...
- ✅ Folder CLAUDE.md system (28 folders)
...
1. **Update Agent Prompts** (5 minutes)
   ...
   "Before file operations, read [folder]/CLAUDE.md for context"
   ```

2. **Increase Max Response Tokens** (1 minute)
   ```typescript
   // In .claude/orchestration/core/config.ts
...
3. **Large codebase exploration**
   ...
   **Solution:** Use folder CLAUDE.md for high-level context
...
// Do this:
agent.read("src/lib/db/schema/CLAUDE.md")  // 2k tokens
...
- ✅ 200k token limit (safety, can't increase beyond Claude max)
- ✅ Folder CLAUDE.md (5x faster context loading)
