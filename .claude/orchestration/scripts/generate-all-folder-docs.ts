#!/usr/bin/env node
/**
 * Generate CLAUDE.md for ALL folders (Maximum Granularity)
 *
...
function generateBasicConfig(folderPath: string): Partial<FolderConfig> {
  ...
  return {
    ...
    developerGuide: `This folder contains ${folderName} related files. Check CLAUDE.md for detailed documentation.`,
    agentGuide: `Auto-generated folder context. Review files in this directory for specific functionality.`,
    importantNotes: ['This is an auto-generated CLAUDE.md - enhance in manual sections as needed'],
  };
}
  ...
  try {
    ...
    if (subdirs.length > 0) {
      ...
      for (const subdir of subdirs.sort((a, b) => a.name.localeCompare(b.name))) {
        list += `- \`${subdir.name}/\` - See [${subdir.name}/CLAUDE.md](${subdir.name}/CLAUDE.md)\n`;
      }
      list += '\n';
    }
 ...
 */
async function generateClaudeMd(config: FolderConfig): Promise<string> {
  ...
  md += `### Recent Changes\n\n`;
  md += `- ${timestamp}: Auto-generated CLAUDE.md created\n\n`;
  md += `<!-- END MANUAL EDIT SECTION -->\n\n`;
  md += `---\n\n`;

  md += `## 📚 Related Documentation\n\n`;
  md += `- [Project Root CLAUDE.md](/CLAUDE.md)\n`;
  md += `- [Project Structure](/PROJECT-STRUCTURE.md)\n`;
  md += `- [Documentation Index](/.claude/DOCUMENTATION-INDEX.md)\n\n`;

  const parentDir = path.dirname(config.path);
  if (parentDir !== '.' && parentDir !== config.path) {
    md += `- [Parent Folder](../${path.basename(parentDir)}/CLAUDE.md)\n`;
  }
...
async function main() {
  console.log('\n╔════════════════════════════════════════════════════════════════════════════╗');
  console.log('║       MAXIMUM GRANULARITY - Generate CLAUDE.md for ALL Folders            ║');
  console.log('╚════════════════════════════════════════════════════════════════════════════╝\n');
  ...
  for (const config of allConfigs) {
    ...
    console.log(`${exists ? '🔄' : '📝'} ${config.path}/CLAUDE.md...`);
...
async function generateMasterIndex(configs: FolderConfig[]) {
  ...
  md += `- [/.claude/orchestration/FILE-STRUCTURE.md](/.claude/orchestration/FILE-STRUCTURE.md) - File-level docs\n`;
  md += `- [/.claude/CLEANUP-SUMMARY.md](/.claude/CLEANUP-SUMMARY.md) - Recent cleanup\n`;
  md += `- [/.claude/FOLDER-DOCS-IMPLEMENTATION.md](/.claude/FOLDER-DOCS-IMPLEMENTATION.md) - This system\n\n`;
  md += `---\n\n`;
  ...
  byTier.strategic.forEach(config => {
    ...
    md += `📄 [View CLAUDE.md](${config.path}/CLAUDE.md)\n\n`;
  });
  md += `---\n\n`;
  ...
  md += `pnpm exec tsx .claude/orchestration/scripts/generate-all-folder-docs.ts\n\n`;
  md += `# View what changed\n`;
  md += `git diff **/CLAUDE.md\n`;
  md += `\`\`\`\n\n`;
