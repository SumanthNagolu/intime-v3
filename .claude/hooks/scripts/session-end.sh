...
# Session End Hook
#
# Automatically captures session data when a Claude Code session ends.
#
# This hook:
...
SESSION_ID=""
if [ -f ".claude/state/current-session.txt" ]; then
    SESSION_ID=$(cat .claude/state/current-session.txt)
else
    # Generate session ID based on date
    SESSION_ID="session-$(date +%Y-%m-%d)-$(uuidgen | cut -c1-8)"
    mkdir -p .claude/state
    echo "$SESSION_ID" > .claude/state/current-session.txt
fi
...
if [ "$AUTO_LOG" = "true" ]; then
    ...
    eval "$CMD" 2>/dev/null || {
        ...
        mkdir -p .claude/state/timeline
        cat > ".claude/state/timeline/${SESSION_ID}-auto.json" <<EOF
{
...
EOF
        echo "  Saved to: .claude/state/timeline/${SESSION_ID}-auto.json"
    }
else
