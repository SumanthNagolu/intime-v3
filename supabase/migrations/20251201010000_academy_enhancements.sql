...
CREATE INDEX idx_learning_paths_deleted_at ON learning_paths(deleted_at) WHERE deleted_at IS NULL;
...
CREATE INDEX idx_learning_path_courses_sequence ON learning_path_courses(sequence);
...
CREATE INDEX idx_path_enrollments_user_id ON path_enrollments(user_id);
CREATE INDEX idx_path_enrollments_path_id ON path_enrollments(path_id);
CREATE INDEX idx_path_enrollments_status ON path_enrollments(status);
...
CREATE INDEX idx_level_definitions_xp_required ON level_definitions(xp_required);
...
CREATE INDEX idx_user_levels_current_xp ON user_levels(current_xp);
...
CREATE INDEX idx_learning_streaks_streak_type ON learning_streaks(streak_type);
CREATE INDEX idx_learning_streaks_current_count ON learning_streaks(current_count);
...
CREATE INDEX idx_leaderboard_entries_xp_earned ON leaderboard_entries(xp_earned);
