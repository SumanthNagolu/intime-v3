...
- [B02-qualify-opportunity.md](./B02-qualify-opportunity.md) - Previous step
- [D01-create-job.md](./D01-create-job.md) - Next step
- [C04-manage-client-relationship.md](./C04-manage-client-relationship.md) - Ongoing
- [C05-conduct-client-meeting.md](./C05-conduct-client-meeting.md) - Ongoing
...
### Multi-Regional Account Setup

Accounts can span multiple regions with region-specific settings:

| Field | Description |
|-------|-------------|
| `primaryRegion` | Account's headquarters region |
| `operatingRegions` | All regions where account operates |
| `billingEntity` | Legal entity for invoicing per region |
| `taxJurisdictions` | Tax ID per country/region |

**Example Multi-Regional Account:**
```
Account: TechCorp Global Inc.
Primary Region: NA (US)
Operating Regions: NA, EMEA, APAC
Legal Entities:
  - NA: TechCorp Inc. (EIN: 12-3456789)
  - EMEA: TechCorp Europe Ltd (VAT: GB123456789)
  - APAC: TechCorp Singapore Pte Ltd (UEN: 202012345A)
```

### Multi-Currency Billing Setup

| Currency Configuration | Purpose |
|------------------------|---------|
| `defaultBillingCurrency` | Primary invoicing currency |
| `acceptedCurrencies` | All currencies client will accept |
| `paymentTerms` | Net 30/45/60 by region |
| `exchangeRatePolicy` | Spot rate, monthly average, locked |

### Regional MSA/Contract Types

| Region | Contract Type | Required Documents |
|--------|---------------|-------------------|
| **US** | MSA + SOW | W-9, COI, NDA |
| **Canada** | MSA + SOW | Business Number, COI |
| **UK** | Framework Agreement | Company Registration, VAT Cert |
| **EU** | EU Services Agreement | VAT ID, GDPR DPA |
| **India** | Service Agreement | GST Registration, PAN |

### Compliance Fields by Region

| Region | Additional Required Fields |
|--------|---------------------------|
| **US** | Supplier Diversity status, EEOC reporting opt-in |
| **Canada** | Indigenous business status, Provincial certifications |
| **EU** | GDPR DPA signed, Data residency requirements |
| **UK** | IR35 determination default, GDPR compliance |
| **APAC** | Local entity requirements, Tax treaties |

### Global Account Health Scoring

Enterprise accounts tracked with regional KPIs:
...
Metrics:
  - Placements (TTM): 47
  - Revenue (TTM): $2.4M USD
  - NPS: 8.5/10
  - Payment Speed: 32 days avg
```

### Regional Owner Assignment

Accounts can have different owners per region:

| Role | NA | EMEA | APAC |
|------|-----|------|------|
| Primary Owner | John Smith | Anna Mueller | Wei Chen |
| Secondary | Sarah Jones | Pierre Dubois | Raj Patel |
| Executive Sponsor | Lisa Director | - | - |
...
```json
{
  ...
  "audit_events": [
    ...
    {
      "action": "account.region_added",
      "timestamp": "2025-12-15T14:00:00Z",
      "new_region": "EMEA",
      "legal_entity": "InTime Europe GmbH"
    }
  ]
}
```
