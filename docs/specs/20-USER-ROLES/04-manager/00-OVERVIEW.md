# UC-MGR-000: Pod Manager Role - Complete Specification
...
The **Pod Manager** is the "path clearer / torch bearer" role in InTime OS. Pod Managers oversee project/client/domain/region-based teams (pods) of Individual Contributors (ICs), enabling their success by removing blockers, handling escalations, and providing strategic guidance.

**Critical Philosophy:** Managers do not do the work themselves - they empower ICs to work independently end-to-end while monitoring performance and stepping in only when needed.

**RACI Position:** Pod Manager is ALWAYS assigned as **Consulted (C)** on all pod objects, ensuring visibility and the ability to provide input before decisions are finalized, without interfering with IC autonomy.
...
|----------|-------|
| Role ID | `pod_manager` |
| Role Type | Leadership / Oversight / Enablement |
| Reports To | Regional Director / COO |
| Oversees | Pod of ICs (Technical Recruiters, Bench Sales, or TA) |
| RACI Default | **Consulted (C)** on ALL pod objects + Informed (I) |
| Pod Structure | Flexible: Project/Client/Domain/Region-based |
| Sprint Focus | Enable pod to hit collective target (1 placement per IC per sprint) |
| Authority Level | Approve submissions, rates, escalations; Coach ICs |
...
## RACI Assignments (Critical Understanding)

**Pod Manager Default Assignment: CONSULTED (C)**

Every business object in a pod has the following RACI structure:
- **Responsible (R):** Primary IC (does the work)
- **Accountable (A):** Secondary IC or Pod Manager (owns the outcome)
- **Consulted (C):** **Pod Manager (ALWAYS)** - provides input before decisions
- **Informed (I):** COO, Regional Director, other stakeholders

### RACI Assignment Matrix

| Scenario | Pod Manager's RACI Role | Actions Required | When to Intervene |
|----------|-------------------------|------------------|-------------------|
| IC creates Job | **Consulted (C)** + Informed (I) | Review job requirements, validate client fit | If job is duplicate, unrealistic, or strategic account |
| IC creates Submission | **Consulted (C)** + Informed (I) | Monitor submission quality and rates | If rate override needed or unusual terms |
| IC schedules Interview | Informed (I) | Notification only, monitor calendar | If conflict with client relationship |
| Offer Extended | **Consulted (C)** + Accountable (A) | **Review and approve offer terms** | Always review before offer goes out |
| Placement Completed | **Consulted (C)** + Informed (I) | Celebrate, update pod metrics, ensure onboarding | If placement has risk factors |
| Client Escalation | Responsible (R) + Accountable (A) | **Take over issue, resolve, inform IC** | Always - escalations are manager's domain |
| Unusual Rate Submission | **Consulted (C)** + Accountable (A) | **Approve or reject with guidance** | Any rate override or negative margin |
| IC Requests Help | **Consulted (C)** | Provide coaching, remove blocker | Immediately when requested |
| Job Closed/Cancelled | **Consulted (C)** | Review reason, validate decision | If premature closure or client satisfaction issue |
| Candidate Rejection | **Consulted (C)** | Monitor rejection reasons | If pattern suggests bias or poor judgment |
