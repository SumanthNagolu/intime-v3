#!/bin/bash
# Session Start Hook - Display project status when Gemini Code starts
...
if [ -d ".gemini/agents" ]; then
  AGENT_COUNT=$(ls -1 .gemini/agents/*.md 2>/dev/null | wc -l | tr -d ' ')
  echo "🤖 Agent System: $AGENT_COUNT agents available"

  if [ "$AGENT_COUNT" -gt 0 ]; then
    echo "   Agents:"
    ls .gemini/agents/*.md 2>/dev/null | xargs -n 1 basename | sed 's/.md$//' | sed 's/^/   - /'
  fi
else
...
# Workflow Commands
if [ -d ".gemini/commands/workflows" ]; then
  CMD_COUNT=$(ls -1 .gemini/commands/workflows/*.md 2>/dev/null | wc -l | tr -d ' ')
  echo "⚡ Workflow Commands: $CMD_COUNT available"

  if [ "$CMD_COUNT" -gt 0 ]; then
    ls .gemini/commands/workflows/*.md 2>/dev/null | xargs -n 1 basename | sed 's/.md$//' | sed 's/^/   \//'
  fi
fi
