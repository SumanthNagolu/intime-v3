 ...
 * - Manual: pnpm exec tsx .gemini/orchestration/scripts/update-project-structure.ts
 */
...
async function gatherMetrics(): Promise<ProjectMetrics> {
  ...
  const orchestrationDir = path.join(process.cwd(), '.gemini/orchestration/core');
...
async function main() {
  ...
  console.log('   - Manually: pnpm exec tsx .gemini/orchestration/scripts/update-project-structure.ts');
  console.log();
}
