#!/usr/bin/env node
/**
 * Generate GEMINI.md files for each directory
 *
...
`,
  ...
  {
    path: '.gemini/orchestration/workflows',
    name: 'Workflows',
...
`,
  ...
  {
    path: '.gemini/orchestration/scripts',
...
# View cleanup report
.gemini/orchestration/scripts/cleanup-report.sh

# Generate folder docs (this creates GEMINI.md files)
pnpm exec tsx .gemini/orchestration/scripts/generate-folder-docs.ts
\`\`\`
`,
    agentGuide: `
...
- generate-folder-docs.ts: Create/update GEMINI.md in folders
`,
    importantNotes: [
  ...
  {
    path: '.gemini/agents/implementation',
...
`,
  ...
  },
  {
    ...
    dependents: [
      ...
      '../../../.gemini/agents/implementation/database-architect - Creates schemas',
    ],
    developerGuide: `
 ...
 */
async function generateGeminiMd(config: DirectoryConfig): Promise<string> {
  ...
  md += `### Recent Changes\n\n`;
  md += `- ${timestamp}: Initial GEMINI.md created\n\n`;
  md += `<!-- END MANUAL EDIT SECTION -->\n\n`;
  md += `---\n\n`;

  md += `## 📚 Related Documentation\n\n`;
  md += `- **Project Root:** [/GEMINI.md](/GEMINI.md) - Main project instructions\n`;
  md += `- **Project Structure:** [/PROJECT-STRUCTURE.md](/PROJECT-STRUCTURE.md) - Complete overview\n`;
  md += `- **File Structure:** [/.gemini/orchestration/FILE-STRUCTURE.md](/.gemini/orchestration/FILE-STRUCTURE.md) - Detailed file docs\n\n`;
  md += `---\n\n`;
 ...
 */
async function main() {
  console.log('📚 Generating folder-specific GEMINI.md files...\n');
  ...
  for (const config of directoryConfigs) {
    ...
    console.log(`📝 Generating ${config.path}/GEMINI.md...`);
...
/**
 * Generate master index of all GEMINI.md files
 */
async function generateMasterIndex() {
  ...
  md += `- [.gemini/orchestration/FILE-STRUCTURE.md](.gemini/orchestration/FILE-STRUCTURE.md) - Detailed file documentation\n`;
  md += `- [.gemini/CLEANUP-SUMMARY.md](.gemini/CLEANUP-SUMMARY.md) - Recent cleanup report\n`;
  md += `- [.gemini/TEST-RESULTS-FINAL-WORKING.md](.gemini/TEST-RESULTS-FINAL-WORKING.md) - Test results\n\n`;
  md += `---\n\n`;

  md += `## 📁 Folder-Specific Documentation (GEMINI.md)\n\n`;

  for (const config of directoryConfigs) {
    const fullPath = path.join(process.cwd(), config.path, 'GEMINI.md');
    if (existsSync(fullPath)) {
      md += `### ${config.name}\n\n`;
      md += `**Path:** \`${config.path}/\`  \n`;
      md += `**Description:** ${config.description}\n\n`;
      md += `📄 [View ${config.path}/GEMINI.md](${config.path}/GEMINI.md)\n\n`;
    }
  }
  ...
  md += `pnpm exec tsx .gemini/orchestration/scripts/update-docs.ts\n\n`;
  md += `# Regenerate folder docs\n`;
  md += `pnpm exec tsx .gemini/orchestration/scripts/generate-folder-docs.ts\n`;
  md += `\`\`\`\n\n`;

  md += `### Git Hook\n`;
  md += `Documentation auto-updates on commit via \`.gemini/hooks/pre-commit-docs\`\n\n`;

  md += `---\n\n`;
  md += `*Auto-generated on ${timestamp}*\n`;

  await fs.writeFile(indexPath, md, 'utf-8');
  console.log(`   ✅ Master index created: .gemini/DOCUMENTATION-INDEX.md\n`);
}
