...
ls -lh .gemini/orchestration/core/*.ts | awk '{printf "  %-30s %8s\n", $9, $5}'
echo ""
echo "Workflows (3 files):"
ls -lh .gemini/orchestration/workflows/*.ts | awk '{printf "  %-30s %8s\n", $9, $5}'
echo ""
echo "Active Tests (3 files):"
ls -lh .gemini/orchestration/test-*.ts 2>/dev/null | awk '{printf "  %-30s %8s\n", $9, $5}'
echo ""
echo "Scripts (1 file):"
ls -lh .gemini/orchestration/scripts/*.ts 2>/dev/null | awk '{printf "  %-30s %8s\n", $9, $5}'
echo ""
...
echo "═══════════════════════════════════════════════════════════════════════════"
echo ""
CORE_LINES=$(find .gemini/orchestration/core -name "*.ts" -exec wc -l {} + | tail -1 | awk '{print $1}')
WORKFLOW_LINES=$(find .gemini/orchestration/workflows -name "*.ts" -exec wc -l {} + | tail -1 | awk '{print $1}')
TEST_LINES=$(find .gemini/orchestration -name "test-*.ts" -exec wc -l {} + 2>/dev/null | tail -1 | awk '{print $1}')
...
if [ -f ".gemini/orchestration/FILE-STRUCTURE.md" ]; then
  echo "  ✅ FILE-STRUCTURE.md ($(stat -f%z .gemini/orchestration/FILE-STRUCTURE.md) bytes)"
else
...
echo "  2. Update documentation: pnpm exec tsx .gemini/orchestration/scripts/update-docs.ts"
