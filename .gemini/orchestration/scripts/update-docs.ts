 ...
 *
 * Run: pnpm exec tsx .gemini/orchestration/scripts/update-docs.ts
 */
  ...
  typeOrder.forEach(type => {
    ...
    files.forEach(file => {
      ...
      if (file.name === 'agent-runner.ts') {
        md += `**Key Features:**\n`;
        md += `- Executes individual agents with Gemini API\n`;
 ...
 */
async function main() {
  console.log('📚 Updating file structure documentation...\n');

  const baseDir = path.join(process.cwd(), '.gemini/orchestration');
  ...
  // Write to file
  const outputPath = path.join(process.cwd(), '.gemini/orchestration/FILE-STRUCTURE.md');
  await fs.writeFile(outputPath, markdown, 'utf-8');

  console.log(`✅ Documentation updated: ${outputPath}\n`);
